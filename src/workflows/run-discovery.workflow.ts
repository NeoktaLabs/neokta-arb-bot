...
function toMarketPools(
  ...
  uniswapPools: Awaited<ReturnType<typeof discoverUniswapPools>>
): MarketPool[] {
...
export async function runDiscoveryWorkflow(
  context: ScanContext
): Promise<DiscoveryWorkflowReport> {
  const curvePools = await discoverCurvePools(context.env, context.chainId);
  const okuPools =
    context.chainId === "etherlink" && context.config.enableOku
      ? await discoverOkuPools(context.env)
      : [];
  const uniswapPools =
    context.chainId === "ethereum" && context.config.enableUniswap
      ? await discoverUniswapPools(context.env, context.config)
      : [];
