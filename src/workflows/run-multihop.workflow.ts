// src/workflows/run-multihop.workflow.ts
...
async function simulatePathSet(
  context: ScanContext,
  paths: ReturnType<typeof generateMultiHopPaths>
): Promise<ClassifiedSimulationResult[]> {
...
export async function runMultiHopWorkflow(
  ...
  const graph = buildTokenGraph(discovery.routePools);
  const paths = generateMultiHopPaths(graph);
  ...
  logInfo("scan.multihop.completed", {
