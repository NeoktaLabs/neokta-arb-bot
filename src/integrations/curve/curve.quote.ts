...
function buildProbeAmounts(decimalsIn: number): bigint[] {
  ...
  if (decimalsIn >= 3) {
    amounts.add(safePow(decimalsIn - 3));
  }

  if (decimalsIn >= 1) {
    amounts.add(safePow(decimalsIn - 1));
  }
...
async function tryMethod(
  ...
  if (method === "get_dy_int128") {
    return getCurveDyInt128(env, chainId, poolAddress as `0x${string}`, i, j, dx);
  }

  return getCurveDyUint256(env, chainId, poolAddress as `0x${string}`, i, j, dx);
}
